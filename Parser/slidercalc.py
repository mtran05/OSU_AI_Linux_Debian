...
def get_end_point(slider_type, slider_length, points):
    ...
    if slider_type == 'linear':
        ...
        try:
            ...
            print("Caught 3 points that are not circle slider (they're colinear)")
            return point_on_line(points[0], points[2], slider_length)
...
def rotate(cx, cy, x, y, radians):  # TODO: Fix
...
def get_circum_circle(p1, p2, p3):
    ...
    # ! Added by Mason 10/26/2024
    if d == 0:
        print(p1, p2, p3)
        raise ValueError("The points are collinear, so the circumcircle is undefined.")
...
# ! DEBUGGING ONLY
# cx, cy, radius = get_circum_circle([45,152], [110,165], [209,92])
# length = 180
# radians = length / radius

# for i in range(101):
#     print(rotate(cx, cy, 45, 152, radians * i / 100))